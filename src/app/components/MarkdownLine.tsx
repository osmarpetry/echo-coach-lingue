...
export default function MarkdownLine({ content, className = '' }: MarkdownLineProps) {
  ...
  if (headingMatch) {
    ...
    return <div className={`${sizeClass} ${className}`}>{parseInline(headingMatch[2])}</div>;
  }
  ...
  // Paragraph
  return <div className={`my-1 ${className}`}>{parseInline(content)}</div>;
}
...
function parseInline(text: string): React.ReactNode {
  ...
  while (remaining.length > 0) {
    ...
    if (boldMatch) {
      parts.push(<strong key={i++} className="font-semibold">{parseInline(boldMatch[1])}</strong>);
    ...
    if (italicMatch) {
      parts.push(<em key={i++} className="italic">{parseInline(italicMatch[1])}</em>);
